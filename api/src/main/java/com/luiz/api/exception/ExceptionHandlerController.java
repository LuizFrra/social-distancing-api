...
public class ExceptionHandlerController {
    ...
    public ResponseEntity<?> catchValidationException(HttpServletRequest req, MethodArgumentNotValidException ex, Locale locale) {
        Map<String, String> errors = new TreeMap<>();

        ex.getBindingResult().getFieldErrors().forEach(error -> {
            errors.put(error.getField(), CustomMessageSource.getMessage(error.getDefaultMessage(), locale));
        });
